...
# собирать по всем городам координаты сотни станций метро и приводить их к нормальному виду было довольно проблематично, и сделано только для москвы
...
def min_distance_to_metro(lat, long):
    distances = [geodesic((lat, long), station).kilometers for station in metro_stations.values()]
    return min(distances)


def process_dataset(file_path):
    data = pd.read_csv(file_path)
    data['distance_to_metro'] = data.apply(lambda row: min_distance_to_metro(row['latitude'], row['longitude']), axis=1)
    data.to_csv(file_path, index=False)


file_path = 'data_Moscow.csv'
